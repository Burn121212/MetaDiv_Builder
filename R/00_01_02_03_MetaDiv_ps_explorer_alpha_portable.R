...
#
# 1) From Anaconda Prompt / terminal:
#    Rscript script.R "C:/path/to/For_R/run_directory"
#
# 2) From RStudio by defining FOR_R_DIR before sourcing the script:
#    FOR_R_DIR <- "C:/path/to/For_R/run_directory"
#    source("script.R")
#
# 3) From RStudio without defining FOR_R_DIR:
#    the script will ask for the input directory interactively.

# Convert Windows paths to their WSL equivalent when necessary.
windows_path_to_wsl <- function(path) {
  
  path <- trimws(path)
  
  # No conversion is required on Windows.
  ...
  # Detect paths such as C:/Users/... or C:\\Users\\...
  if (grepl("^[A-Za-z]:/", path)) {
    
    drive_letter <- tolower(substr(path, 1, 1))
    path_without_drive <- substring(path, 3)
    ...
    message(
      "[MetaDiv] Windows path converted for WSL: ",
      path
    )
  }
...
resolve_for_r_dir <- function() {
  ...
  # 1. Directory supplied from the command line.
  if (length(args) >= 1 && nzchar(args[1])) {
    
    selected_dir <- windows_path_to_wsl(args[1])
  ...
  # 2. Existing FOR_R_DIR variable, useful in RStudio.
  if (
    ...
    selected_dir <- windows_path_to_wsl(FOR_R_DIR)
  ...
  # 3. Interactive directory selection on Windows.
  ...
  # 4. Manual entry in an interactive R session.
  if (interactive()) {
    ...
    if (!nzchar(selected_dir)) {
      stop("No input directory was provided.", call. = FALSE)
    }
...
# All R outputs are saved inside the exact input directory.
# This keeps each analysis linked to its own MetaDiv run configuration
# (for example: subset, SINTAX threshold and SPPN threshold).
#
# Example:
# .../For_R/all_eukaryotes/genus_p1_sppn08/
#     abundance_table.csv
#     taxonomy_table.csv
#     sample_metadata.csv
#     sequences.fasta
...
missing_required_files <- required_input_files[
...
for (file_path in required_input_files) {
  cat("[REQUIRED: OK] ", file_path, "
", sep = "")
}

for (file_path in optional_input_files) {
  status <- if (file.exists(file_path)) "[OPTIONAL: OK] " else "[OPTIONAL: NOT FOUND] "
  cat(status, file_path, "
", sep = "")
}
